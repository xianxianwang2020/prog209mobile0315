...
var ItemObject = function ( pID,pName, pCategory,pBrand,pPrice, pOwner,pImageLink) {
  this.ID=pID;
  this.Name = pName;
  ...
  $(document).on("pagebeforeshow", "#detailPage", function (event) {   // have to use jQuery 
    ...
    for(let i=0; i < itemArray.length; i++) {   
      if (localID==itemArray[i].ID){
        ...
        document.getElementById("oneImage").innerHTML= itemArray[i].ImageLink;
      }
...
function createList()
{
  ...
  itemArray.forEach(function (element,) {   // use handy array forEach method
    ...
    li.innerHTML = "<a data-transition='pop' class='oneItem' data-parm=" +
     ...
     +element.Name+" ||   "+"         "+element.Category+"        ||       "+element.Brand;
...
function addNewItem(newItem){
    ...
    fetch(request)
        ...
        .then(theResonsePromiseJson => console.log(theResonsePromiseJson), document.location.href = "#Consign" )
